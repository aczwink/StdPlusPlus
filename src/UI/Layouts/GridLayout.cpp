...
{
	//TODO: try to shrink where widget allow it
	float64 shrinkHeightPerRow = totalShrinkHeight / rowHeights.GetNumberOfElements();
	for (uint16 &rowHeight : rowHeights)
	{
		rowHeight -= shrinkHeightPerRow;
	}
}