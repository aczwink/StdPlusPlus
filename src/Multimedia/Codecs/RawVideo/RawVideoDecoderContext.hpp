...
class RawVideoDecoderContext : public StdXX::Multimedia::DecoderContext
{
public:
	//Constructor
	inline RawVideoDecoderContext(Stream &stream) : DecoderContext(stream)
	{
	}
	...
	void Decode(const IPacket &packet) override
	{
		...
		const auto& size = this->stream.codingParameters.video.size;
